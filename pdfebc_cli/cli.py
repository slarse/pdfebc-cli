...
import sys
import os
from pdfebc_core import config_utils
...
See 'https://github.com/slarse/pdfebc-cli' for more information.
"""
...
def create_argparser():
    ...
    out_dir_default = config_utils.get_attribute_from_config(
        config, config_utils.DEFAULT_SECTION_KEY, config_utils.OUT_DEFAULT_DIR_KEY)
    src_dir_default = config_utils.get_attribute_from_config(
        config, config_utils.DEFAULT_SECTION_KEY, config_utils.SRC_DEFAULT_DIR_KEY)
    gs_default_binary = config_utils.get_attribute_from_config(
...
def diagnose_config():
    ...
    if not os.path.isfile(config_utils.CONFIG_PATH):
        status_callback(NO_CONFIG_FOUND.format(config_utils.CONFIG_PATH))
        return
    config = config_utils.read_config()
    config_path, missing_sections, malformed_entries = config_utils.run_config_diagnostics()
    output = []
    ...
    config_string = config_utils.config_to_string(config)
